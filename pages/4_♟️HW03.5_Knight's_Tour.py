...
random_input = st.checkbox(
    label="隨機起點", value=True
)
...
if not random_input:
    x_input = st.number_input(label="row", min_value=0, max_value=size_input - 1, value=0)
    y_input = st.number_input(label="column", min_value=0, max_value=size_input - 1, value=0)
