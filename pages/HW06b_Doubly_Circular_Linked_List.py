...
from homework import DoublyCircularLinkedList
...
st.header("雙向循環鏈結串列")
st.caption("Doubly Circular Linked List")
...
with c1:
    ...
    input_location = st.radio(
        "location",
        input_options_location,
        format_func=lambda x: input_options_location.get(x),
        horizontal=True,
        index=1 if input_action == "create" else 0 if input_action == "search" else 2,
    )
        ...
        with col2:
            if input_location in ["target", "before", "after"]:
                input_target = st.number_input(
                    "target", min_value=0, max_value=2**31 - 1
                )
    ...
    try:
        ...
        if res == -1:
            if input_location in ["head", "tail"]:
                ...
                raise ValueError(f"找不到目標數字 {target}")
        if res == -2:
            raise ValueError(f"{input_options_location.get(input_location)}沒有數字")
    except ValueError as e:
        st.toast(e)
